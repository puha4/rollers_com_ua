...
class Tmnewsletter extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('TemplateMonster Newsletter');
	...
	public function getContent()
	{
		...
		else if (Tools::isSubmit('submitTmnewsletterGuestModule'))
		{
		...
		else if (Tools::isSubmit('resetUserCookies'))
		{
		...
		else if (Tools::isSubmit('resetGuestCookies'))
		{
		...
		foreach (Language::getLanguages(false) as $lang)
		{
			if (isset($result['id_tmnewsletter']))
			{
				$sql_lang = 'SELECT `title`, `content`
							...
							WHERE `id_tmnewsletter` = '.$result['id_tmnewsletter'].'
			...
			$data['TMNEWSLETTER_'.$type_name.'_MESSAGE'][$lang['id_lang']] = isset($lang_data['content']) ? Tools::htmlentitiesDecodeUTF8($lang_data['content']) : '';
		}
	...
	protected function postProcess($is_guest = false)
	{
		...
		$sql = 'SELECT `id_tmnewsletter`
				...
				AND `is_guest` = '.$type_val;

		if ($item_id = Db::getInstance()->getValue($sql))
		{
			if (!Db::getInstance()->delete('tmnewsletter_settings', '`id_tmnewsletter` = '.$item_id.'')
				|| !Db::getInstance()->delete('tmnewsletter_settings_lang', '`id_tmnewsletter` = '.$item_id.''))
				return false;
		}
		...
		if (!Db::getInstance()->insert('tmnewsletter_settings', array(
												...
												'timeout' => Tools::getValue('TMNEWSLETTER_'.$type_name.'_TIMEOUT'),
												'ft_delay' => Tools::getValue('TMNEWSLETTER_'.$type_name.'_FTDELAY'),
		...
		foreach (Language::getLanguages(false) as $lang)
		{
			if (!Db::getInstance()->insert('tmnewsletter_settings_lang', array(
												...
												'content' => Tools::htmlentitiesUTF8(Tools::getValue('TMNEWSLETTER_'.$type_name.'_MESSAGE_'.$lang['id_lang']))
												)))
				return false;
		}
	...
	protected function getPopupContent($is_guest = false)
	{
		$sql = 'SELECT tmnsl.`title`, tmnsl.`content`
				...
				WHERE tmns.`id_shop` = '.$this->context->shop->id.'
				AND tmnsl.`id_lang` = '.$this->context->language->id.'
	...
	private function checkLatestShow()
	{
		...
		if ($this->context->customer->id)
		{
			...
			// don't show message if customer blocked it
			else if ($result['status'])
				return false;
			// display message if not db entire or if delay time expired
			else if ($this->compareTime($this->context->cookie->last_newsletter_showed) > 0)
				return true;
			else
				return false;
		}
		// checking - guest's latest message shows
		else
		{
			$sql = 'SELECT `status`
					FROM '._DB_PREFIX_.'tmnewsletter
			...
			// don't show message if guest blocked it
			else if ($result['status'])
				return false;
			// display message if not db entire or if delay time expired
			else if ($this->compareTime($this->context->cookie->last_newsletter_showed) > 0)
	...
	private function clearCookies($is_guest = false)
	{
		...
		{
			// clear db entires
			if (!Db::getInstance()->delete('tmnewsletter', '`id_shop` = '.$this->context->shop->id.' AND `id_user` != 0'))
				return false;
		}
		else
		{
			// clear db entires
			if (!Db::getInstance()->delete('tmnewsletter', '`id_shop` = '.$this->context->shop->id.' AND `id_guest` != 0'))
				return false;
		}
	...
	public function hookHeader()
	{
			...
			&& !Tools::getValue('content_only'))
		{
			...
			if (!$this->checkUser())
			{
				...
				else if (!$context->cookie->last_newsletter_showed && !$context->cookie->user_come_time)
				{
				...
				$content = $this->getPopupContent(true);
				$this->context->smarty->assign('title', $content['title']);
				$this->context->smarty->assign('content', Tools::htmlentitiesDecodeUTF8($content['content']));
			}
			else
			{
				// blocking popup if user spend not enough time
