...
class  smartakciiDetailsModuleFrontController extends smartakciiModuleFrontController
{
    public $ssl = true;
    public $_report = '';
	...
	public function initContent()
	{
		   ...
		   $slug = Tools::getValue('slug');
			$id_post = $SmartAkcii->slug2id($slug);
           
		   $id_lang = $this->context->language->id;
           $id_lang_defaut = Configuration::get('PS_LANG_DEFAULT');
           $post = $SmartAkciiPost->getPost($id_post,$id_lang);
           $tags = $SmartAkciiPost->getProductTags($id_post);
           $comment = $akciicomment->getComment($id_post);
           $countcomment = $akciicomment->getToltalComment($id_post);
           $id_cate = $post['id_category'];
           $title_category = $AkciiCategory->getNameCategory($id_cate);
           if(file_exists(_PS_MODULE_DIR_.'smartakcii/images/' . $id_post . '.jpg') )
			{
				$post_img =  $id_post;
			}else{
				$post_img = 'no';
	   ...
	   SmartAkciiPost::postViewed($id_post);
	   ...
	   $this->context->smarty->assign('HOOK_SMART_AKCII_POST_FOOTER',
	...
	private function smartsendMail($sname,$semailAddr,$scomment,$slink = null)
    {
			...
			$e_body ='You have Received a New Comment In Your Akcii Post From '. $name . '. Comment: '.$scomment.' .Your Can reply Here : '.$slink.'';
