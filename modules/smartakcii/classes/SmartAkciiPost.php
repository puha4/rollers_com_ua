...
class SmartAkciiPost extends ObjectModel
{
      public $id_smart_akcii_post;
      public $id_author;
      public $id_category;
      public $position = 0;
      public $active = 1;
      ...
      public static $definition = array(
		'table' => 'smart_akcii_post',
		'primary' => 'id_smart_akcii_post',
                'multilang'=>true,
		'fields' => array(
    ...
    public function __construct($id = null, $id_lang = null, $id_shop = null){
            Shop::addTableAssociation('smart_akcii_post', array('type' => 'shop'));
    ...
    public static function getPost($id_post,$id_lang = null){
        ...
        $sql = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_post p INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_lang pl ON p.id_smart_akcii_post=pl.id_smart_akcii_post INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_shop ps ON pl.id_smart_akcii_post = ps.id_smart_akcii_post 
                WHERE pl.id_lang='.$id_lang.'
                AND p.active= 1 AND p.id_smart_akcii_post = '.$id_post;
        
        if (!$post = Db::getInstance()->executeS($sql))
			return false;
                $result['id_post'] = $post[0]['id_smart_akcii_post'];
                ...
                if (file_exists(_PS_MODULE_DIR_.'smartakcii/images/' . $post[0]['id_smart_akcii_post'] . '.jpg') )
                {
                   $image =   $post[0]['id_smart_akcii_post'] . '.jpg';
    ...
    public static function getAllPost($id_lang = null,$limit_start,$limit){
        ...
        $sql = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_post p INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_lang pl ON p.id_smart_akcii_post=pl.id_smart_akcii_post INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_shop ps ON pl.id_smart_akcii_post = ps.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id.'
                WHERE pl.id_lang='.$id_lang.'
                AND p.active= 1 ORDER BY p.id_smart_akcii_post DESC LIMIT '.$limit_start.','.$limit;
        ...
        $i = 0;
            foreach($posts as $post){
                $result[$i]['id_post'] = $post['id_smart_akcii_post'];
                ...
                $result[$i]['cat_name'] = $AkciiCategory->getCatName($post['id_category']);
                $result[$i]['cat_link_rewrite'] = $AkciiCategory->getCatLinkRewrite($post['id_category']);
                $employee = new  Employee( $post['id_author']);
                
                $result[$i]['lastname'] = $employee->lastname;
                $result[$i]['firstname'] = $employee->firstname;
                 if (file_exists(_PS_MODULE_DIR_.'smartakcii/images/' . $post['id_smart_akcii_post'] . '.jpg'))
                    {
                       $image =   $post['id_smart_akcii_post'];
    ...
    public static function addTags($id_lang = null, $id_post, $tag_list, $separator = ','){
		...
		if (is_array($tag_list))
			...
			{
                              $id_tag = AkciiTag::TagExists($tag,(int)$id_lang);
                                if(!$id_tag)
                                 {
                                       $tag_obj = new AkciiTag(null, $tag, (int)$id_lang);
		...
		INSERT INTO `'._DB_PREFIX_.'smart_akcii_post_tag` (`id_tag`, `id_post`)
		VALUES '.$data);
	}
    ...
    public static function postViewed($id_post){
	    ...
	    $sql = 'UPDATE '._DB_PREFIX_.'smart_akcii_post as p SET p.viewed = (p.viewed+1) where p.id_smart_akcii_post = '.$id_post;
    ...
    public function setProducts($array){
		$result = Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'smart_akcii_post_tag WHERE id_tag = '.(int)$this->id);
		if (is_array($array))
		{
			$array = array_map('intval', $array);
			$result &= ObjectModel::updateMultishopTable('smart_akcii_post_tag', array('indexed' => 0), 'a.id_post IN ('.implode(',', $array).')');
			$ids = array();
			...
			if ($result)
			{
				$result &= Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'smart_akcii_post_tag (id_post, id_tag) VALUES '.implode(',', $ids));
    ...
    public static function deleteTagsForProduct($id_post){
		return Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'smart_akcii_post_tag` WHERE `id_post` = '.(int)$id_post);
	}
    ...
    public static function getProductTags($id_post){
		...
		SELECT  t.`name`
		FROM '._DB_PREFIX_.'smart_akcii_tag t
		LEFT JOIN '._DB_PREFIX_.'smart_akcii_post_tag pt ON (pt.id_tag = t.id_tag AND t.id_lang = '.$id_lang.')
    ...
    public static function getPopularPosts($id_lang= null){
                ...
                if(Configuration::get('smartshowpopularpost') != '' && Configuration::get('smartshowpopularpost') != null){
                    ...
                    $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT p.viewed ,p.created , p.id_smart_akcii_post,pl.meta_title,pl.link_rewrite FROM '._DB_PREFIX_.'smart_akcii_post p INNER JOIN 
                    '._DB_PREFIX_.'smart_akcii_post_lang pl ON p.id_smart_akcii_post=pl.id_smart_akcii_post INNER JOIN 
                    '._DB_PREFIX_.'smart_akcii_post_shop ps ON pl.id_smart_akcii_post = ps.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id.'
    ...
    public static function getRelatedPosts($id_lang = null,$id_cat = null,$id_post= null){
         ...
         if(Configuration::get('smartshowrelatedpost') != '' && Configuration::get('smartshowrelatedpost') != null){
            ...
            $sql = 'SELECT  p.id_smart_akcii_post,p.created,pl.meta_title,pl.link_rewrite FROM '._DB_PREFIX_.'smart_akcii_post p INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_lang pl ON p.id_smart_akcii_post=pl.id_smart_akcii_post INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_shop ps ON pl.id_smart_akcii_post = ps.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id.'
                WHERE pl.id_lang='.$id_lang.'  AND p.active = 1 AND p.id_category = '.$id_cat.' AND p.id_smart_akcii_post != '.$id_post.' ORDER BY p.id_smart_akcii_post DESC LIMIT 0,'.$limit;
    ...
    public static function getRecentPosts($id_lang = null){
		...
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT  p.id_smart_akcii_post,p.created,pl.meta_title,pl.link_rewrite FROM '._DB_PREFIX_.'smart_akcii_post p INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_lang pl ON p.id_smart_akcii_post=pl.id_smart_akcii_post INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_shop ps ON pl.id_smart_akcii_post = ps.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id.'
                WHERE pl.id_lang='.$id_lang.'  AND p.active = 1 ORDER BY p.id_smart_akcii_post DESC LIMIT 0,'.$limit);
    ...
    public static function tagsPost($tags, $id_lang = null){
        ...
        $sql = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_post p INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_lang pl ON p.id_smart_akcii_post=pl.id_smart_akcii_post INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_shop ps ON p.id_smart_akcii_post=ps.id_smart_akcii_post  AND  ps.id_shop = '.(int) Context::getContext()->shop->id.' INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_tag pt ON pl.id_smart_akcii_post = pt.id_post INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_tag t ON pt.id_tag=t.id_tag 
    ...
    public static function getArchiveResult($month = null,$year = null,$limit_start = 0, $limit = 5){
         ...
         {
             $sql = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_post s INNER JOIN '._DB_PREFIX_.'smart_akcii_post_lang sl ON s.id_smart_akcii_post = sl.id_smart_akcii_post
                 and sl.id_lang = '.$id_lang.' INNER JOIN '._DB_PREFIX_.'smart_akcii_post_shop ps ON ps.id_smart_akcii_post = s.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id. '
            where s.active = 1 and MONTH(s.created) = '.$month.' AND YEAR(s.created) = '.$year.' ORDER BY s.id_smart_akcii_post DESC';
         }elseif($month == '' and $month == NULL and $year != '' and $year != NULL)
         {
              $sql = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_post s INNER JOIN '._DB_PREFIX_.'smart_akcii_post_lang sl ON s.id_smart_akcii_post = sl.id_smart_akcii_post
                 and sl.id_lang = '.$id_lang.' INNER JOIN '._DB_PREFIX_.'smart_akcii_post_shop ps ON ps.id_smart_akcii_post = s.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id. '
           where s.active = 1 AND YEAR(s.created) = '.$year.' ORDER BY s.id_smart_akcii_post DESC';
              
         }elseif($month != '' and $month != NULL and $year == '' and $year == NULL)
         {
               $sql = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_post s INNER JOIN '._DB_PREFIX_.'smart_akcii_post_lang sl ON s.id_smart_akcii_post = sl.id_smart_akcii_post
                 and sl.id_lang = '.$id_lang.' INNER JOIN '._DB_PREFIX_.'smart_akcii_post_shop ps ON ps.id_smart_akcii_post = s.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id. '
           where s.active = 1 AND   MONTH(s.created) = '.$month.'  ORDER BY s.id_smart_akcii_post DESC';
               
         }else{
             $sql = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_post s INNER JOIN '._DB_PREFIX_.'smart_akcii_post_lang sl ON s.id_smart_akcii_post = sl.id_smart_akcii_post
                 and sl.id_lang = '.$id_lang.' INNER JOIN '._DB_PREFIX_.'smart_akcii_post_shop ps ON ps.id_smart_akcii_post = s.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id. '
            where s.active = 1 ORDER BY s.id_smart_akcii_post DESC';
         }
        if (!$posts = Db::getInstance()->executeS($sql))
			return false;
    ...
    public static function getArchiveD($month,$year){
        ...
        $sql = 'SELECT DAY(p.created) as day FROM '._DB_PREFIX_.'smart_akcii_post p INNER JOIN '._DB_PREFIX_.'smart_akcii_post_shop ps ON p.id_smart_akcii_post = ps.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id.' 
    ...
    public static function getArchiveM($year){
        ...
        $sql = 'SELECT MONTH(p.created) as month FROM '._DB_PREFIX_.'smart_akcii_post p  INNER JOIN '._DB_PREFIX_.'smart_akcii_post_shop ps ON p.id_smart_akcii_post = ps.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id.' 
    ...
    public static function getArchive(){
         $result = array();
        $sql = 'SELECT YEAR(p.created) as year FROM '._DB_PREFIX_.'smart_akcii_post p INNER JOIN '._DB_PREFIX_.'smart_akcii_post_shop ps ON p.id_smart_akcii_post = ps.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id.' 
        ...
        if (!$posts = Db::getInstance()->executeS($sql))
			return false;
                    ...
                    foreach ($posts as $value) {
                       ...
                       $result[$i]['month'] = SmartAkciiPost::getArchiveM($value['year']);
                       $months = SmartAkciiPost::getArchiveM($value['year']);
                       $j = 0;
                       foreach ($months as $month) {
                               $result[$i]['month'][$j]['day'] = SmartAkciiPost::getArchiveD($month['month'],$value['year']);
                               $j++;
                        }
                       $i++;
                    }
    ...
    public static function SmartAkciiSearchPost($keyword = NULL, $id_lang = NULL,$limit_start = 0, $limit = 5){
          if($keyword == NULL)
              return false;
          if($id_lang == NULL)
              $id_lang = (int)Context::getContext()->language->id;
          
        $sql = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_post_lang pl, '._DB_PREFIX_.'smart_akcii_post p 
                WHERE pl.id_lang='.$id_lang.'  AND p.active = 1 
                AND pl.id_smart_akcii_post=p.id_smart_akcii_post AND
                (pl.meta_title LIKE \'%'.$keyword.'%\' OR
                 pl.meta_keyword LIKE \'%'.$keyword.'%\' OR
                 pl.meta_description LIKE \'%'.$keyword.'%\' OR
                 pl.content LIKE \'%'.$keyword.'%\') ORDER BY p.id_smart_akcii_post DESC';
       if (!$posts = Db::getInstance()->executeS($sql))
			return false;
    ...
    public static function SmartAkciiSearchPostCount($keyword = NULL, $id_lang = NULL){
          if($keyword == NULL)
              return false;
          if($id_lang == NULL)
              $id_lang = (int)Context::getContext()->language->id;
        ...
        $sql = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_post_lang pl, '._DB_PREFIX_.'smart_akcii_post p 
                WHERE pl.id_lang='.$id_lang.'
                AND pl.id_smart_akcii_post=p.id_smart_akcii_post AND p.active = 1 AND 
                (pl.meta_title LIKE \'%'.$keyword.'%\' OR
                 pl.meta_keyword LIKE \'%'.$keyword.'%\' OR
                 pl.meta_description LIKE \'%'.$keyword.'%\' OR
                 pl.content LIKE \'%'.$keyword.'%\') ORDER BY p.id_smart_akcii_post DESC';
    ...
    public static function getAkciiImage(){
         
         $id_lang = (int)Context::getContext()->language->id;
            
                $sql = 'SELECT id_smart_akcii_post FROM '._DB_PREFIX_.'smart_akcii_post';
    ...
    public static function GetPostMetaByPost($id_post,$id_lang = null){
        ...
        public static function GetPostLatestHome($limit)
            ...
            $sql = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_post p INNER JOIN 
                ...
                '._DB_PREFIX_.'smart_akcii_post_shop ps ON pl.id_smart_akcii_post = ps.id_smart_akcii_post AND ps.id_shop = '.(int) Context::getContext()->shop->id.'
                WHERE pl.id_lang='.$id_lang.' 		
                AND p.active= 1 ORDER BY p.id_smart_akcii_post DESC 
                LIMIT '.$limit;
            $posts = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
            if(empty($posts)){
                $sql2 = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_post p INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_lang pl ON p.id_smart_akcii_post=pl.id_smart_akcii_post INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_shop ps ON pl.id_smart_akcii_post = ps.id_smart_akcii_post  AND ps.id_shop = '.(int) Context::getContext()->shop->id.'
             ...
             $i = 0;
            foreach($posts as $post){
                $result[$i]['id'] = $post['id_smart_akcii_post'];
                ...
                $result[$i]['is_featured'] = $post['is_featured'];
                $result[$i]['link_rewrite'] = $post['link_rewrite'];
				$result[$i]['countcomment'] = $akciicomment->getToltalComment($post['id_smart_akcii_post']);
                if (file_exists(_PS_MODULE_DIR_.'smartakcii/images/' . $post['id_smart_akcii_post'] . '.jpg'))
                    {
                       $image =   $post['id_smart_akcii_post'];
                       $result[$i]['post_img'] = $image;
