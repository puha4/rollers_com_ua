...
class Akciicomment extends ObjectModel
{
      public $id_smart_akcii_comment;
      public $id_parent;
      public $id_customer;
      public $id_post;
      public $name;
      ...
      public static $definition = array(
		'table' => 'smart_akcii_comment',
		'primary' => 'id_smart_akcii_comment',
                'multilang'=>false,
		'fields' => array(
    ...
    public function __construct($id = null, $id_shop = null)
            {
            Shop::addTableAssociation('smart_akcii_comment', array('type' => 'shop'));
    ...
    public   function addComment($id_post,$comment, $value,$id_parent){
        ...
        $sql = 'INSERT INTO '._DB_PREFIX_.'smart_akcii_comment(id_post,name,email,content,website,id_parent, active) VALUES ('.(int)$id_post.', \''.$comment['name'].'\', \''.$comment['mail'].'\', \''.$comment['comment'].'\', \''.$comment['website'].'\', '.$id_parent .', '.$value.')';
        ...
        $sql = 'SELECT * FROM '._DB_PREFIX_.'smart_akcii_comment 
        ...
        foreach($child_comments as $ch_comment){
             ...
             if($this->comment_child_loop<=$this->comment_child_loop_depth) {
                 $coments_2 = $this->getChildComment($ch_comment['id_smart_akcii_comment']);
    ...
    public  function getComment($id_post){
        ...
        foreach($comments as $comment){
              $coments =$this->getChildComment($comment['id_smart_akcii_comment']);
    ...
    public static function getLatestComments($id_lang = null){
            ...
            if($id_lang == null){
                ...
                $sql = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT * FROM '._DB_PREFIX_.'smart_akcii_comment bc INNER JOIN 
                '._DB_PREFIX_.'smart_akcii_post_shop bps ON bc.id_post=bps.id_smart_akcii_post and bps.id_shop = '.(int) Context::getContext()->shop->id.'
                WHERE  bc.active= 1 ORDER BY bc.id_smart_akcii_comment DESC limit 0, '.$limit);
		
                 $i = 0;
            foreach($sql as $post){
                $result[$i]['id_smart_akcii_comment'] = $post['id_smart_akcii_comment'];
                ...
                $result[$i]['slug'] = $SmartAkciiPost->GetPostSlugById($post['id_post']);
                $i++;
            }
		return $result;
    }
     ...
     public static function getToltalComment($id){
        ...
        $sql = 'SELECT id_post FROM '._DB_PREFIX_.'smart_akcii_comment 
                WHERE id_post='.$id.' 
                AND active=1';
