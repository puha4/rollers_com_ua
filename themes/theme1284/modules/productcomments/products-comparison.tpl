        	...
        	<td class="{$classname} comparison_infos ajax_block_product product-{$id_product}" align="center">
        		{if isset($tab_grade[$id_product]) AND $tab_grade[$id_product]}
        			<div class="product-rating">
                        ...
                        <input class="star" disabled="disabled" type="radio" name="{$grade_id}_{$id_product}_{$smarty.section.average.index}" {if isset($tab_grade[$id_product]) AND $tab_grade[$id_product]|round neq 0 and $smarty.section.average.index <= $tab_grade[$id_product]|round}checked="checked"{/if} />
        				{/section}
        			</div>
        		{else}
        			-
	...
	{foreach from=$list_ids_product item=id_product}
		<td class="{$classname} comparison_infos product-{$id_product}" align="center" >
			{if isset($list_product_average[$id_product]) AND $list_product_average[$id_product]}
				<div class="product-rating">
					{section loop=6 step=1 start=1 name=averages}
                    <input class="auto-submit-star" disabled="disabled" type="radio" name="average_{$id_product}_{$smarty.section.averages.index}" {if $list_product_average[$id_product]|round neq 0 AND $smarty.section.averages.index <= $list_product_average[$id_product]|round} checked="checked"{/if} />
					{/section}
				</div>
			{else}
				-
    ...
    {foreach from=$list_ids_product item=id_product}
    	<td class="{$classname} comparison_infos product-{$id_product}" align="center" >
    		{if isset($product_comments[$id_product]) AND $product_comments[$id_product]}
    			<a class="btn btn-default btn-sm icon-right" href="#" data-id-product-comment="{$id_product|intval}" rel="#comments_{$id_product|intval}" title="{l s='View comments' mod='productcomments'}">
    				<span>
